		...
		for _, check_offset := range *offsets {
			...
			if historyHash != nil {
				if len(*historyHash) == 3 && *historyHash == eofhash {
					...
					//isDup = true
					if his.L1Cache != nil {
						//his.L1Cache.Set(*hash, "1", DefaultCacheExpires)
...
func (his *HISTORY) boltBucketPutBatch(db *bolt.DB, char string, bucket string, batchQueue chan *BatchOffset, forced bool, src string) (err error) {
	...
	if forced {
		logf(DEBUG9,"FORCED SYNCING boltBucketPutBatch char=%s buk=%s", char, bucket)
		for {
			time.Sleep(time.Second/10)
			if len(batchQueue) == 0 {
				logf(DEBUG9,"DONE SYNCING boltBucketPutBatch char=%s buk=%s", char, bucket)
				break
			}
...
func (his *HISTORY) boltBucketGetOffsets(db *bolt.DB, char *string, bucket *string, key *string) (offsets *[]int64, err error) {
	...
	*/
	...
	if len(gobEncodedOffsets) > 0 {
		...
		go his.Sync_upcounter("decodedOffsets")
		offsets = decodedOffsets
...
func gobDecodeOffsets(encodedData []byte, src string) (*[]int64, error) {
	...
	if err != nil {
		log.Printf("ERROR gobDecodeOffsets encodedData='%#v' len=%d decodedOffsets='%#v' err='%v' src=%s", encodedData, len(encodedData), decodedOffsets, err, src)
		return nil, err
	}
...
func (his *HISTORY) L1CacheSet(hash *string, value string) {
	//logf(DEBUG1, "L1CacheSet hash=%s value=%s", *hash, value)
	his.cache_mux.Lock()
	his.L1Cache.Set(*hash, value, DefaultCacheExpires)
	his.cache_mux.Unlock()
} // end func L1CacheSet

func (his *HISTORY) CheckL1Cache(hash *string) (int) {
	if his.L1Cache != nil { // check go-cache for hash
		his.cache_mux.Lock()
		defer his.cache_mux.Unlock()
		if val, found := his.L1Cache.Get(*hash); found {
			switch val {
			case "-1":
				return -1
			...
			case "-2":
				...
				//cacheretry++
			default:
				log.Printf("ERROR CheckL1Cache unknown switch val=%d", val)
				return -999
			}
			/*
			switch val {
			...
			*/
			...
			his.L1Cache.Set(*hash, "-1", DefaultCacheExpires) // adds hash to temporary go-cache with value "-1"
			return 0
		}
	} else {
