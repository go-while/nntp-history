...
fetchbatch:
	...
	if len(batch1) > 0 {
		...
		logf(DEBUG, "INFO bboltPutBatch [%s|%s] B=%05d inserted=%05d lft=%d wCBBS=%d ...... f=%t ( took %d micros ) ", char, bucket, len(batch1), inserted1, lastflush, workerCharBucketBatchSize, forced, insert1_took)
